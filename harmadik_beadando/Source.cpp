...
char			window_title[] = "Harmadik beadando - AZO5LO";
...
float vertices2[] = { //also kocka - hozzáadott
	// positions          normals
...
float vertices3[] = { //hozzáadott harmadik kocka
	// positions          normals
	-0.5f, -0.5f, 1.5f,  0.0f,  0.0f, -1.0f,
	 0.5f, -0.5f, 1.5f,  0.0f,  0.0f, -1.0f,
	 0.5f,  0.5f, 1.5f,  0.0f,  0.0f, -1.0f,
...
GLfloat		kor_var = 0.0;  
GLfloat		r = 9.0; //kezdõpozícióhoz az r (8 <= r <= 10)

/** Modell, vetítési és kamera mátrixok felvétele. */
glm::mat4	model, view, projection = glm::perspective(glm::radians(55.0f), (float)window_width / (float)window_height, 0.1f, 100.0f); //45-rõl 55-re átállítva
...
glm::vec3	cameraPos		= glm::vec3( r, 0.0f, 0.0f), //r hozzáadva
			cameraTarget	= glm::vec3( 0.0f, 0.0f, 0.0f),
			up	= glm::vec3( 0.0f, 0.0f, 1.0f), //UP vektor, át kell majd nevezni!
			cameraMovingX	= glm::vec3(0.0f, 0.0f, 1.0f), //ez lesz a Z (X volt, de arra nincs szükség)
...
//	rotateM	= glm::rotate(glm::mat4(1.0f), glm::radians(45.0f), glm::vec3(0.0f, 1.0f, 0.0f));
	scaleM	= glm::scale(glm::mat4(1.0f), glm::vec3(1.0f, 1.0f, 1.0f)); //a középsõ érték 0.5 volt
...
void display() {
	...
	if ((keyboard[GLFW_KEY_W]) || (keyboard[GLFW_KEY_UP])) //Y volt
		cameraPos += cameraSpeed * cameraMovingX;

	if ((keyboard[GLFW_KEY_S]) || (keyboard[GLFW_KEY_DOWN])) //Y volt
		cameraPos -= cameraSpeed * cameraMovingX;




	if ((keyboard[GLFW_KEY_A]) || (keyboard[GLFW_KEY_LEFT])) { //megváltoztatott - órán elhangzottak alapján
		//cameraPos += cameraSpeed * cameraMovingY;
		kor_var -= cameraSpeed;			
		cameraPos.x = r * cos(kor_var);
		cameraPos.y = r * sin(kor_var);
	}
		

	if ((keyboard[GLFW_KEY_D]) || (keyboard[GLFW_KEY_RIGHT])) { //megváltozatott - órán elhangzottak alapján
		//cameraPos -= cameraSpeed * cameraMovingY;
		kor_var += cameraSpeed;
		cameraPos.x = r * cos(kor_var);
		cameraPos.y = r * sin(kor_var);
	...
	//második - hozzáadott
	glBindVertexArray(VAO[1]);
	glDrawArrays(GL_TRIANGLES, 0, sizeof(vertices2) / (6 * 4));

	//harmadik - hozzáadott
	glBindVertexArray(VAO[2]);
	glDrawArrays(GL_TRIANGLES, 0, sizeof(vertices3) / (6 * 4));
